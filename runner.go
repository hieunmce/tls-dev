...
var (
	...
	deterministic      = flag.Bool("deterministic", false, "If true, uses a deterministic PRNG in the tls.")
...
func gettlsCertificate(t testCert) Certificate {
...
type testCase struct {
	...
	// message before doing a roundtrip with the tls.
	shimWritesFirst bool
	// readWithUnfinishedWrite behaves like shimWritesFirst, but the shim
	// does not complete the write until responding to the first tls
	// message.
	...
	// messages from the tls. The tls will default to not sending
	// application data.
	shimShutsDown bool
...
func doExchange(test *testCase, config *Config, conn net.Conn, isResume bool, transcripts *[][]byte, num int) error {
	...
	for j := 0; j < messageCount; j++ {
		...
		// Process the KeyUpdate ACK. However many KeyUpdates the tls
					...
					if vers.version >= VersionTLS13 {
						tests = append(tests, testCase{
							...
							flags: append([]string{
								...
								"-reverify-on-resume",
								"-on-resume-shim-writes-first",
								// Session tickets are disabled, so the tls will not send a ticket.
...
`,
		...
		// Bidirectional shutdown with the tls initiating.
		tests = append(tests, testCase{
			name: "Shutdown-tls",
			config: Config{
				Bugs: ProtocolBugs{
		...
		if !config.implicitHandshake {
			// Bidirectional shutdown with the shim initiating. The tls,
...
func addVersionNegotiationTests() {
	for _, protocol := range []protocol{tls, dtls} {
		for _, shimVers := range allVersions(protocol) {
			...
			// Test configuring the tls's maximum version.
			for _, tlsVers := range allVersions(protocol) {
				expectedVersion := shimVers.version
				if tlsVers.version < shimVers.version {
					expectedVersion = tlsVers.version
				}

				suffix := shimVers.name + "-" + tlsVers.name
				if protocol == dtls {
					suffix += "-DTLS"
				}
				...
				testCases = append(testCases, testCase{
					...
					config: Config{
						...
						Bugs: ProtocolBugs{
							...
							NegotiateVersion:            tlsVers.wire(protocol),
							IgnorePeerCipherPreferences: shouldFail,
						},
					},
					flags:              flags,
...
func addExtensionTests() {
	...
	// offered in ClientHello. The tls's server will fail if this occurs,
...
func addResumptionVersionTests() {
	...
	testCases = append(testCases, testCase{
		...
		config: Config{
			...
			Bugs: ProtocolBugs{
				FilterTicket: func(in []byte) ([]byte, error) {
					// If the client (tls) offers ChaCha20-Poly1305 first, the
...
func addSignatureAlgorithmTests() {
	...
	for _, alg := range testSignatureAlgorithms {
		for _, ver := range tlsVersions {
			...
			testCases = append(testCases, testCase{
				...
				config: Config{
					MaxVersion:   ver.version,
					Certificates: []Certificate{gettlsCertificate(alg.cert)},
	...
	// Test that the client advertises a set which the tls can find
	...
	rsaPSSSupportTests := []struct {
		...
		// (This is checked internally in the tls.)
		{
			name:                "ConfigNoPSS",
...
func addDTLSRetransmitTests() {
	...
	// the tls.
	//
	// TimeoutSchedule configures the tls to send a series of timeout
	...
	// tls bracket each handshake flight.
	//
	...
	//
	// The tls then drops all packets received before the ACK and
	...
	// post-handshake penultimate Finished from the tls. The above tests
...
func addCustomExtensionTests() {
	...
	testCases = append(testCases, testCase{
		...
		// get successfully decrypted by the tls.
...
func addWrongMessageTypeTests() {
	for _, t := range makePerMessageTests() {
		...
		if t.test.config.MaxVersion >= VersionTLS13 && t.messageType == typeServerHello {
			...
			// encryption, so the alert is not readable by tls.
